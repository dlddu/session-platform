...
// rule: read, write and switch all bring the session to active first —
// promoting idle or restoring a snapshot — and then serve from the live pod.
package service
...
// Service is the concrete Manager. It owns no workload itself: pod operations
// go through the orchestrator and state mutations through the store.
type Service struct {
...
// shell strategy is the constructor's ckpt argument.
...
// "Active" means more than pod Ready: the selected workload agent must pass the
// control-plane reachability check before the session is stored.
func (s *Service) Create(ctx context.Context, req session.CreateRequest) (*session.Session, error) {
	...
	if err := s.orch.Reach(ctx, pod); err != nil {
		...
		// reclaim it instead of leaking it.
...
// from a live pod, and reports which branch brought it there. This is the shared
// core of the uniform "resume-on-access" policy.
func (s *Service) activate(ctx context.Context, id string) (*session.Session, string, error) {
	...
	case session.StateIdle:
		// Idle still holds its pod; resume it to active atomically.
...
// offset, plus the nextOffset cursor. Offset 0 replays the full session history;
...
// sends the payload to the agent. The call returns after acceptance; output is
// recovered via Read.
...
// snapshot as needed — and is a no-op for an already-active session. It shares
// the activate core with Read/Write so all three resume a session identically.
...
// Snapshot checkpoints an active/idle session and reclaims its pod. Explicit
// snapshots have no idle precondition.
...
// Guarded by the per-session lock so concurrent restores are atomic.
func (s *Service) Restore(ctx context.Context, id string) (result *session.Session, retErr error) {
	...
	// Both restore paths preserve the byte stream, so a pre-snapshot read cursor
	// remains valid. The restore pod runs the session's own workload type, which is
	// fixed at creation and never changed by a restore.
	workload, err := session.NormalizeWorkloadType(sess.WorkloadType)
	if err != nil {
		return nil, err
	}
	...
	// workload agent is reachable again.
	if err := s.orch.Reach(ctx, pod); err != nil {
		s.stopPodBestEffort(pod)
		return nil, err
	}
...
// Terminate reclaims the pod (if any) and removes the session. It uses the same
// per-session Lease as snapshot/restore so deletion cannot race a restore target
// into existence or remove metadata underneath a snapshot transaction.
...
// resume from, or "" when there is no checkpoint. Snapshot always records one
// before a session reaches StateSnapshot, so the nil case is defensive.
...
// DispatchPath renders the Path label for the branch activate took, e.g.
// "idle->active->read" or "snapshot->restore->write".
