...
//     를 넘을 때만 돌고, 이 SUT 에는 무자격 GET 에 그만한 본문을 돌려주는 인클러스터 origin 이
//     하나도 없다 — deploy/kustomization.yaml 을 전수로 읽어 확인했다. 세 대역이 GET 에 내는
//     것은 /healthz 와 게이트웨이의 작은 JSON 뿐이고(나머지 라우트는 전부 POST), 실물로 선 둘도
//     마찬가지다: minio.yaml 은 익명 읽기 정책이 없어 서명 없는 GET 이 작은 403 XML 로 끝나고,
//     plugin-marketplace-git.yaml 이 smart HTTP 로 내는 저장소는 marketplace.json 과 픽스처
//     SKILL.md 넷뿐이라 클론 전체가 그 선에 못 미친다. 그런 origin 을 세우는 것은 대역 확장이고
//     소관은 tbm_session-platform-e2e-mock-policy(차단 요인 FETCH-ORIGIN)다. 그래서
