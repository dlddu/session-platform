...
| `CLAUDE-PROVIDER` | `EXT` | claude-code 세션의 프롬프트 왕복에서 **응답을 만드는 주체 하나만** 인클러스터 대역(`deploy/e2e-anthropic-fake.yaml`)으로 바꾸고 나머지는 전부 실물이다. 요청은 주 컨테이너가 오케스트레이터가 주입한 loopback `ANTHROPIC_BASE_URL`로 보내고, credential-proxy 사이드카가 **프로덕션 코드 그대로** 헤더 허용목록을 적용하고 호출자가 붙인 `Authorization`을 버린 뒤 자기 Secret 환경의 플랫폼 토큰을 주입하며, 대역의 인증서를 **실제로 검증한다** — 사설 발급자를 신뢰하는 근거는 플랫폼 Secret의 optional `ca-cert` 키 하나뿐이고 시스템 루트는 대체되지 않는다. 대역은 bearer가 틀리면 401을 내므로 주입이 실제로 일어났는지가 대역 쪽에서도 확인된다. `control-plane/test/e2e_provider_reachability_test.go`가 배포 SUT의 세션 pod 안에서 이 왕복을 **버퍼드·SSE 두 형태**로 단언하고, 위조 `Authorization`이 대체되는 것까지 확인한다. **2026-09-04부터 실 CLI의 프롬프트 루프 자체가 이 표면 위에서 구동된다** — `control-plane/test/e2e_e2_prompt_invocation_test.go`가 배포 SUT의 세션 pod에서 write 한 번당 `claude` 프로세스를 실제로 기동시켜, 플랫폼이 만든 exact argv·원샷 수명·직렬 큐를 프로세스 테이블에서 관측하고 그 응답이 세션의 append-only 출력에 투영되는 것까지 단언한다(AC-E2). **2026-09-11 부터 도구 호출 갈래도 이 표면 위에서 관측된다** — `e2e_provider_reachability_test.go` 의 `TestProviderEmitsAToolUseBlockWhenThePromptAsksForOne` 이 배포 SUT 의 세션 pod 에서 프록시를 거쳐 `tool_use` 블록과 `stop_reason: "tool_use"` 를, 스트리밍에서는 `input_json_delta` 를 단언한다. | **실 provider의 계약은 여전히 미검증**이다 — 오류 응답·토큰 회계·모델 라우팅은 대역이 결정적 상수로 답하므로 드러나지 않는다. 프롬프트 루프가 이 표면으로 **성립한다는 것**은 위 구간이 확인했다. 여기 적혀 있던 잔여 둘 중 ⑵ **한 invocation이 내는 `content_block_delta`가 1개다** 는 **2026-09-10 에 해소됐다** — 대역이 프롬프트의 `e2e-reply:` 지시자에 따라 본문을 여러 델타로 나눠 내고, `claude-code-workload.md#시나리오 4` 의 파일이 그 출력이 64 KiB 청크 한계에서 **문자 경계로 잘려** 두 개의 output 이벤트로 나오는 것까지 단언한다. ⑴ 은 **좁아졌지만 남는다**: 응답은 이제 프롬프트의 **라벨과 크기**를 반영하지만 여전히 그 **의미**는 반영하지 않으므로, AC-E4(대화 연속성)와 AC-E5 의 「동결 전 대화를 참조하는 프롬프트」는 이 대역으로 단언할 수 없다(그래서 `claude-code-workload.md#시나리오 5` 는 예외 목록에 그대로 있다). 여기 적혀 있던 셋째 공백 — 대역이 `tool_use` 블록을 내지 못해 **모델이 도구를 부르는 갈래**가 이 표면에서 시작되지 않던 것 — 은 **2026-09-11 에 닫혔다**. 대역이 프롬프트의 `e2e-tool:<도구명>:<JSON 입력>` 지시자에 따라 `tool_use` 블록을 버퍼드·SSE 두 형태로 내고 `stop_reason` 도 `tool_use` 로 답하므로 `approval-gated-workload.md#시나리오 3` 의 승인 왕복이 **진입 가능해졌다**(그래서 「차단 요인」에서 그 행이 나갔다). **그 자리에 남는 잔여는 다른 것이다**: 대역은 도구를 **지시받아서** 부르지 판단해서 부르지 않는다. *언제* 부르는가, 도구 결과를 받아 *무엇을 이어서 하는가*(결과 턴은 지시자를 담지 않아 상수 응답으로 닫힌다), 그리고 text 와 tool_use 가 **한 메시지에 섞이는** 실제 형태 — 셋 다 이 표면으로는 단언되지 않는다. 대역이 내는 도구 이름이 세션 MCP 가 실제로 등재한 것과 같은지도 여기서는 확인되지 않는다(지시자가 이름을 축자로 나른다) — 그 대조는 승인 왕복을 끝까지 도는 시나리오 파일의 몫이고, 저작은 `tbm_session-platform-scenario-e2e` 소관이다. |
| `APPROVAL-GATEWAY` | `EXT` | `approval-gated` 세션의 승인 왕복에서 **결정을 내리는 주체(사람)** 하나만 인클러스터 대역(`deploy/e2e-approval-gateway-fake.yaml`)으로 바꾸고 나머지는 전부 실물이다. 헬퍼 파드의 MCP 컨테이너가 **프로덕션 코드 그대로** 승인 요청을 만들고(`POST /api/requests` + `x-api-key`, 외부 식별자 `{세션ID}:{요청ID}`), 3초 간격으로 폴링하며, `APPROVED` 일 때만 실제 외부 GET 을 한다. 거절·만료는 `isError` 도구 결과로 돌아가고, 비-2xx 는 결정이 아니라 게이트웨이 오류로 갈린다. 대역이 `x-api-key` 를 검사해 틀리면 401 을 내므로 게이트웨이 키가 **MCP 컨테이너에만** 있다는 것도 대역 쪽에서 확인된다(AC-F6). 결정을 고르는 면은 대역 자신의 운영자 라우트(`/operator/*`)이고 제품에 뚫은 test 전용 트리거가 아니다 — 그래서 `TRIG` 가 아니라 `EXT` 다. | 실 게이트웨이의 **알림 전달과 만료 시계**는 미검증이다 — 사람이 무엇을 보고 무엇을 눌렀는지, 게이트웨이가 스스로 `EXPIRED` 로 넘기는 시점이 언제인지는 여기서 드러나지 않는다. **TIMEOUT 갈래도 이 대역으로 열리지 않는다**: `defaultApprovalTimeout`(10분)·폴링 간격(3초)이 `approval_gateway.go` 에 하드코딩이라 결정되지 않는 요청을 내도 e2e 가 10분을 태워야 한다. 그 설정화는 제품 변경이라 이 표의 소관이 아니다. 그리고 **승인 갈래조차 「게이트를 통과했다」는 데까지만 관측된다** — 결재가 떨어진 뒤의 실제 아웃바운드 호출은 이 대역이 서도 여전히 막혀 있다(아래 「차단 요인」). 이 대역이 산 것은 *세션이 서고 승인 왕복이 돈다*이지 *승인된 도구가 끝까지 간다*가 아니다. |
...
**새 파일이 이미 승인된 인터셉트를 재사용할 때 — 선행 task 가 아니라 같은 hunk 다.** 최상위
시나리오 파일을 새로 저작하면서 이미 등재된 `NET` 기법(예: `STREAM-RESET-REPLAY` 의 `/stream`·
`/read` 좁힌 인터셉트)을 그대로 쓰는 경우, 필요한 것은 **그 CODE 의 회계 행을 새 파일로 늘리고
`mock-exception:` 마커를 그 파일에 다는 것**이고 그것은 **저작 PR 자신의 hunk 에서 끝난다**. R5·R7
이 회계 행을 요구하므로 그 한 hunk 를 빼먹으면 CI 가 막지만, 그것은 저작을 막는 *선행*이 아니라
저작에 **딸린 절차**다. 새 CODE 를 만들 필요도 없다 — 같은 기법·같은 자격이면 같은 CODE 에
귀속시키는 쪽이 「예외는 늘지 않는 방향으로만」에 맞는다. 그러므로 `claude-code-workload.md#시나리오 9`·
`lifecycle.md#시나리오 4` 의 저작은 **이 절의 별도 작업을 기다리지 않는다**(그 두 행이 「모킹 허용목록
등재가 선행」이라 적고 이 절을 소관으로 지목하고 있으나, 여기서 미리 내주는 답이 그것이다).
아래 「차단 요인」에 행을 두지 않는 이유도 같다 — 인터셉트 기법이 이미 실재해 e2e 가 그 경로를
**밟고 있으므로** 차단 요인의 정의에 해당하지 않고, 따라서 해소 시 사라질 리터럴도 없다.
...
### 차단 요인 (seam이 아니다 — e2e가 그 경로를 밟지 못하는 원인)

위 「미해소 위반」이 *치환이 있어서* 생긴 drift라면, 여기 있는 것은 e2e가 그 경로를 **아예 밟지
못하는** 이유다. 형태는 둘이고 **둘 다 이 표에 온다**:

1. **치환조차 없는 공백** — 세울 대역도 실물도 아직 없어서 경로가 시작되지 않는다.
2. **등재된 치환의 표현력 부족** — 치환은 있고 등재돼 있는데 그것이 내지 못하는 것이 있어
   경로에 **진입조차 못 한다**. 치환이 있다는 이유로 이것을 표 밖에 두면, 등재↔코드가 1:1인
   동안 아무 게이트도 그 공백을 세지 않는다(그것이 2026-09-11에 이 표가 한 행 늘어난 이유다).

claude-code 워크로드 AC(AC-E1~E6)가 「AC 검증 범위」에서
통째로 공백이던 근거가 이 표였다 — 세 행이 차례로 나가면서 AC-E1이 먼저 공백을 벗었고, 그 뒤로
전용 파일이 하나씩 붙고 있다(2026-09-08 축 교체 뒤로 **무엇이 공백인지는 「집계」의
`scenario-summary` 블록이 정본**이고 단위는 AC가 아니라 시나리오다 — 여기에 옮겨 적지 않는다). seam이 아니므로 코드에
...
**표는 2026-09-06에 다시 채워졌다.** 셋이 나가고 비어 있던 자리에 한 행이 들어왔다 — 예고한
그대로, 새로 발견되는 "치환조차 없는 공백"의 원인은 여기에 다시 쌓인다. 이번 행은 approval-gated
세션의 **결재 이후** 구간이다. 게이트웨이 대역이 서면서 「이 타입의 세션이 아예 서지 못한다」는
원인 자체는 `APPROVAL-GATEWAY` 등재로 사라졌지만, 승인이 떨어진 뒤의 실제 아웃바운드 호출은
여전히 치환조차 없는 공백이다 — 그래서 seam 이 아니라 여기에 온다. 판정이 `없음` 인 것은 이
공백을 메우는 것이 대역이 아니라 **실물 배포**이기 때문이다: 정적 문서를 내는 https 원본은
MinIO · 마켓플레이스 원격과 같은 성격이라 kind 에 실제로 세울 수 있고, 그러면 정의의 EXT 배제
조항이 그대로 걸린다.

**개수는 gap이 아니다 — 재는 것은 해소 계획의 존재다.** 차단 요인은 대부분 즉시 해소할 수 없어서
개수를 그대로 빨간불로 쓰면 영구 red가 되고 신호가 죽는다. 그래서 행마다 **소관 · 선행 · 재검토
시점** 세 칸을 두고, 그것을 **산문이 아니라 칸으로** 요구한다 — 소관·선행이 '해소 시' 산문에
섞여 있으면 사람은 읽어도 게이트와 정합성 감지는 못 읽어 인계가 조용히 유실된다(실제로
2026-09-10까지 그랬다). 형태는 `scripts/check-fidelity-allowlist.py`의 R11이 강제한다:

- **소관** — 그 방향을 실행할 **받는 쪽**. 값은 정합성 모델 id(`tbm_…`) 또는 `이 모델`. 「제품
  변경이라 이 표의 소관이 아니다」처럼 **부정문만 있는 것은 소관이 아니다** — 넘기려면 받는 쪽을
  지목해야 한다.
- **선행** — 먼저 풀려야 하는 것. 없으면 `없음`이라 적고, 그러면 그 행은 **착수 가능**이다
  (`선행: 없음` + `소관: 이 모델`인 행이 있으면 그 모델의 다음 감지가 작업을 연다).
- **재검토 시점** — ISO 날짜(`YYYY-MM-DD`) 또는 관측 가능한 사건(task id `rct_…` · PR 번호 `#NN` ·
  백틱 코드 리터럴). 「언젠가 제품이 바뀌면」처럼 관측 불가능한 것은 재검토 시점이 아니다.
  날짜로 적으면 등재일로부터 **최장 90일**을 넘기지 않는다.

⚠️ **게이트는 형태만 본다 — 날짜가 지났는지는 보지 않는다.** 경과 판정은 정합성 감지
(`tbm_session-platform-e2e-mock-policy`)의 몫이고, 거기서 작업을 여는 것이 제자리다. CI를 시계로
빨갛게 만들면 그 빨강은 이 PR과 무관한 사람에게 떨어진다. 90일 상한도 같은 이유로 게이트가 세지
않는다(등재일이 표에 없어 기계적으로 잴 수 없다는 것도 함께 사실이다).

<!-- fidelity:blockers -->
| 무엇이 막는가 | 코드 위치 | 리터럴 | 판정 | 소관 | 선행 | 재검토 시점 | 해소 시 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 승인된 `web_fetch_get` 의 **실제 아웃바운드 호출**. 그 호출을 내는 MCP 컨테이너의 fetch 는 **시스템 루트만 신뢰하는 기본 클라이언트**다 — credential-proxy 가 optional `ca-cert` 키로 사설 발급자를 신뢰하는 것과 달리, 헬퍼 pod 의 MCP 컨테이너에는 CA 를 넣을 자리가 아예 없다(`helperPodSpec` 이 넣는 env 는 워크로드·주소·세션ID·게이트웨이 3종뿐이다). 그래서 인클러스터 **https** 원본을 세워도 그 인증서를 검증할 수 없고, 승인 갈래는 *게이트를 통과했다*는 데까지만 관측된다. ⚠️ **전제 정정 — 이 행이 들고 있던 「도구는 https 만 받는다(`validateFetchTarget`)」는 2026-09-12 에 거짓이 됐다**(#129): `validateFetchTarget` 은 이제 `http` 와 `https` 를 함께 받는다(`url must be http or https`). 「인클러스터 원본은 https 여야 한다」는 고리가 끊기면서 **검증할 인증서가 애초에 없는 평문 http 원본**이 선택지에 들어왔다 — 실제로 그 길로 갈지는 아래 『해소 시』의 ①②③ 과 함께 이 행의 `선행`(해소 방향 재판정)에 속하고 **이 정정은 그 판정을 하지 않는다**. 여기서 고친 것은 전제 한 문장이며, 달라진 것은 그 재판정의 **입력**이다. | `data-plane/cmd/agent/session_mcp_tools.go` | `http.DefaultClient.Do(req)` | `없음` | `이 모델` | 해소 방향 재판정 | 2026-10-11 | MCP 컨테이너가 사설 발급자를 신뢰할 길이 먼저 생겨야 한다(제품 변경 — credential-proxy 의 optional `ca-cert` 와 같은 형태). 그 길이 생기면 정적 문서를 내는 https 원본을 **실물로 배포**해 등재 없이 닫는다(MinIO · 마켓플레이스 원격 선례). 그때 이 리터럴은 자기 CA 풀을 든 클라이언트로 바뀌면서 사라지고, 이 행도 함께 지운다. 반대로 실물이 아니라 대역이어야 한다고 판정이 뒤집히면 판정을 EXT 로 고쳐 `FETCH-ORIGIN` 으로 등재하고 이 행을 지운다. ⚠️ **그 뒤집힘은 2026-09-07에 이미 일어났고 아직 적용되지 않았다** — 제품에 CA 주입을 스테이징한 PR #93이 검토에서 반려되며 해소를 대역으로 가기로 정해졌고, `docs/doc-tracker.md` 의 🟠 인계 절이 「이 행이 적고 있던 `tbm_session-platform-docs-impl` 귀속은 그 판정 **이전**의 것이며 그 정정도 등재와 같은 hunk 에서 **그 모델이** 한다」고 이 표를 지목한다. 그래서 2026-09-11에 소관을 `이 모델` 로 고쳤다. **판정은 아직 뒤집지 않았다**: 대역을 세워도 같은 행의 『무엇이 막는가』가 그대로 걸리기 때문이다(사설 CA 를 검증할 자리가 MCP 컨테이너에 없다). 남은 선택은 셋이고 — ① 공개 https 원본을 실물로 보아 시스템 루트 풀로 닫는다 ② 대역 + CA 주입 경로를 재검토한다(PR #93 의 반려 근거가 여전히 유효한지 포함) ③ 관측 지점을 세션 런타임 밖으로 옮긴다 — 그 판정이 이 행의 `선행` 이다. |
<!-- /fidelity:blockers -->
...
| `deploy/e2e-approval-gateway-fake.yaml` | `test-only` | `APPROVAL-GATEWAY` | 인클러스터 승인 게이트웨이 대역. `CLAUDE-PROVIDER` 와 같은 근거로 EXT 다 — PRD 가 지목한 참조 구현(`dlddu/pure-agent`)이 싣고 있는 것은 게이트키퍼 **클라이언트**뿐이고 승인 API 서버는 그 레포에 없어, 세울 실물이 없다. 평문 HTTP 인 것도 의도다: 게이트웨이 클라이언트는 스킴을 강제하지 않으므로 인증서를 흉내 낼 근거가 없다(도구의 대상 URL 쪽도 2026-09-12 부터 `http`·`https` 를 함께 받는다 — `validateFetchTarget`). 같은 파일이 내는 `approval-gateway-credentials` Secret 은 치환이 아니라 `claude-code-credentials` 와 같은 성격의 SUT 자격 픽스처다. |
...
- 차단 요인 **1**건 (seam이 아니다 — 승인 뒤의 아웃바운드 호출)
- 착수 가능 **0**건 — `선행: 없음` + `소관: 이 모델` 인 행. 남은 1건은 `선행` 이 「해소 방향 재판정」이라 아직 착수 가능이 아니다
<!-- /fidelity:summary -->
...
```bash
make check-scenario-mapping          # 시나리오 ↔ e2e 1:1 정합성 (정적 — 클러스터 불필요)
make e2e-up                          # kind 생성 + 이미지 빌드/load + deploy/ 적용 + 헬스 대기
cd control-plane && go test -tags=e2e ./test/...   # API e2e (시나리오별 전용 파일 + 스모크)
...
이 문서는 **테스트 시나리오 ↔ e2e 파일 매핑의 등재 SSOT**다. 시나리오 자체의 SSOT는 이 문서를
제외한 `docs/test/*.md`의 `### 시나리오 N: …` 헤딩이고, 매핑의 SSOT는 각 매칭 단위 파일 헤더의
`// 검증 시나리오:` 선언이다. 이 절의 표들은 그 둘을 잇는 등재이며,
`scripts/e2e/check-scenario-mapping.sh`가 셋의 일치를 기계적으로 강제한다
(`make check-scenario-mapping`, ci.yml에서 모든 PR에 실행 — 클러스터 불필요).

**시나리오 식별자**는 `<문서 파일명>#시나리오 <N>`이고 `<N>`은 **헤딩이 스스로 단 번호 토큰**이다 —
`approval-gated-workload.md#시나리오 2-1`처럼 하위 번호가 실재하므로 헤딩에서 그대로 베껴 온다.
...
> 옮겼다. 옛 AC 키는 아래 표의 「기존 AC 키(참고)」 칸에 남겨 두었지만 **게이트가 읽지 않는
> 참고값**이다 — 시나리오와 AC의 연결에 대한 정본은 각 `docs/test/*.md` 시나리오의 `**검증 AC**`
> 줄이고, 둘이 갈리면 그쪽이 옳다.
...
e2e 자동 검증이 곤란해 전용 파일을 두지 않는 시나리오. 등재된 시나리오는 파일이 없어도 위반이 아니다.

<!-- scenario-exceptions:begin -->
| 시나리오 | 사유 | 대체 검증 수단 |
| --- | --- | --- |
...
| `approval-gated-workload.md#시나리오 6` | **Playwright 이관 미완료 — kube 관측 의존.** 기대 결과가 두 컨테이너의 같은 마운트와 **파드를 건너는 파일 왕복**이라 브라우저로도 control-plane HTTP API 로도 관측되지 않는다. 규칙 4의 **예외 전환 후보**이며, 어느 쪽인지는 이 행을 집는 슬라이스가 판정한다. 개정 전에는 「저작 대기」(대체 검증 없음)였고, **2026-09-13 에 Go 대체 검증이 들어와 미저작에서 이관으로 옮겨졌다**. | `control-plane/test/e2e_approval_gated_6_shared_volume_test.go` | 이 모델 | `web/e2e/` 최상위 전용 spec 착지 **또는** 규칙 4 예외로 전환(대체 검증 수단 = `control-plane/test/e2e_approval_gated_6_shared_volume_test.go`). 어느 쪽이든 이 행을 같은 PR 에서 제거한다. 이관을 택하면 관측 경로를 최소 테스트 계측으로 내야 하고, 그것이 제품 동작을 바꾸면 경계를 넘은 것이라 예외로 간다. 남은 선행 정보: **없음** (2026-09-12 에 풀렸다). 선행이던 「시나리오 3 의 승인 경로 파일」은 `control-plane/test/e2e_approval_gated_3_approval_path_test.go` 가 세웠고, 공유 볼륨에 파일을 만드는 수단도 실재한다 — `session_mcp_tools.go` 의 `spillBody` 가 `maxInlineBodyBytes`(100 000) 초과 응답을 `bodyPath`/`bodyBytes` 로 볼륨에 떨군다(AC-F5 전반, #101). ⚠️ **(c) 복원 갈래만은 아직 못 산다** — 이 타입은 `POST /snapshot` 이 503 `checkpoint strategy is disabled` 라(`ErrCheckpointDisabled`) 동결 자체가 없다. 그 한 갈래는 AC-F5 아카이브 전략이 선행이고 소관은 `tbm_session-platform-docs-impl` 이다. | RWX 볼륨은 #98·#100으로 착지했다 — `control-plane/internal/adapter/k8s/shared_volume.go`의 `ReadWriteMany` 클레임, `deploy/shared-volume-provisioner.yaml`(local-path `sharedFileSystemPath`), `deploy/kind-config.yaml`의 전 노드 동일 마운트, `control-plane/test/shared_volume_test.go`·`approval_gated_shared_volume_test.go`. **「RWX가 미구현이라 막혔다」는 더 이상 참이 아니다.** **대체 검증이 실제로 사는 것**(2026-09-13 착지, 배포 SUT 에서 관측): 승인된 응답이 떨어지는 자리에 놓인 파일을 **워크로드 파드의 에이전트가 자기 도구로** 읽어 볼륨에 되쓰고 그 바이트를 헬퍼 파드의 MCP 컨테이너가 다시 읽는다 — 왕복이 파드를 **두 번** 건너고 그 사이에 동결이 없다(시나리오 6 (b)) · 그 난스가 **세션 출력 버퍼에는 한 번도 없다**(전달이 도구 응답 본문이 아니라 볼륨으로 이뤄진다는 음성 단언, 대역 상수 응답을 대조군으로 먼저 통과시킨다) · (e) 다른 세션의 워크로드는 같은 상대 경로에서 그것을 보지 못한다(자기 세션의 워크로드는 본다는 양성 대조군 뒤에). 마운트 층(ReadWriteMany bind · 같은 경로 · `credential-proxy` 미마운트 · 세션별 클레임 · 동반 회수)은 `control-plane/test/shared_volume_test.go` 가 이미 사므로 대체 검증은 그것을 되사지 않는다. **세 갈래는 아직 없다** — (a) 앞머리의 「승인된 외부 호출로 파일을 만들게 함」 · (c) 복원 후 잔존 · (d) 동결 전·후 커서. 셋 다 아래 §「남은 미검증 분기」에 등재했다. |
...
> **이 표가 비는 날 완전 1:1이다.** 현재 잔여는 두 갈래다 — 이관 28(매칭 공간 축소가 만든 부채, 검증은
> 이미 있다)과 미저작 6(개정 전부터의 잔여, 검증이 아직 없다). 둘을 한 표에 두는 것은 해제 방법이
> 다르기 때문이 아니라 **집계에 세는 방법이 같기** 때문이다. 어느 쪽인지는 `현행 대체 검증` 칸이
> 말한다 — 채워져 있으면 이관, `없음`이면 미저작이다.

## 비-시나리오 파일 등재

시나리오 대신 스모크/인프라를 검증하는 매칭 단위 파일(규칙 3).
...
아래는 e2e로 돌지만 **시나리오 매칭 공간 밖**이므로 1:1 계산에 들어가지 않는다.
...
뒤에도 게이트 초록 아래에서 계속 AC-E2를 공백으로 세고 있었다.
...
>
> 아래 문단들은 **2026-09-08 축 교체 이전** AC 축에서 「왜 이 AC가 공백인가」를 판정한 기록이다.
...
> **나머지 E 계열의 선행도 같은 날 풀렸다.** 필요한 것은 **프롬프트에 대한 응답**이었고,
> 인클러스터 provider 대역을 세워 `CLAUDE-PROVIDER`(EXT)로 등재하면서 「차단 요인」 표가 비었다.
> 그래서 아직 공백으로 남은 E 계열의 이유는 이제 **원인이 아니라 순서**다 — 전용 파일을 저작하는
> 일이 남아 있다. 그 슬라이스가 먼저 확인해야 했던 것(실 Claude CLI가 그 대역으로 프롬프트 루프를
...
> 밖으로 두는 이유는 둘이다. ① j6 여정 spec(2026-09-05에 `j6-agent-prompt-loop`·
> `j6-model-selection`·`j6-stream-recovery` 세 파일로 나뉘었다)이 **파일마다 여러 AC의 화면을
> 걸쳐** 훑어, 규칙 2(파일당 AC 1개)를 만족시키려면 AC별 분할이라는 별도 슬라이스가 필요하다.
> ② *(2026-09-05 갱신 — 이 근거는 해소됐다)* 원래는 j6와 `manual-archive`가 위 「미해소 위반」
...
> 참고). 남은 다섯 건에 대해 두 갈래를 다 검토했고 **(판정 시점 기준) 어느 쪽도 성립하지 않았다** —
> 어느 AC가 공백인지의 정본이던 `ac-summary` 블록은 2026-09-08 축 교체로 사라졌다. 지금 무엇이
> 공백인지는 위 `scenario-summary` 블록(단위: 시나리오)이고, 이 문단은 어느 목록의 사본도 두지 않는다.
>
...
>   **AC-F2(정책을 집행하는 CNI)와 AC-F5(동결·복원을 건너는 보존)는 각자의 선행에 그대로 막혀 있다**
...
>   **AC-F5의 공유 볼륨은 그 뒤 착지해 `control-plane/test/shared_volume_test.go`가 배포 SUT에서
>   덮게 됐고**(남은 후반은 아래 ⚠️), 정책의 **실집행**은 여전히
...
> - **AC-F5는 여기에 들어가지 않는다** — 아래 ⚠️가 적는 대로 이 타입에 아카이브 전략이 등록돼
>   있지 않은 것이 선행이다.
...
| 승인 컨텍스트에 자격 증명·게이트웨이 키가 실리지 않음 | 〃 | **선행은 2026-09-12 에 풀렸다** — 「승인 왕복 자체가 아직 없다」가 이 행의 막힌 이유였는데, `e2e_approval_gated_3_approval_path_test.go` 가 그 왕복을 세워 게이트웨이 대역에 실제 요청 레코드가 생기고 그 레코드의 `context` 를 대역의 `/operator/requests` 로 읽을 수 있다. 남은 것은 **저작**이며 소유 파일은 그대로 f6 이다(승인 컨텍스트는 자격 증명 배치의 조항이지 승인 경로의 조항이 아니다 — 두 파일에 담으면 규칙 1 의 중복이 된다). 토큰이 **조회 응답·목록 응답·`read` 응답·control-plane 로그**에 나타나지 않는 것은 소유 파일이 이미 산다 |
...
| 승인 후 **실제 아웃바운드 GET 이 정확히 1회** 일어나고 그 응답이 같은 바이트열에 이어 붙음 | `e2e_approval_gated_3_approval_path_test.go` | 시나리오 3 사전 조건의 「호출을 관찰할 수 있는 외부 테스트 upstream」이 이 SUT 에 **없다**. 그 호출을 내는 MCP 컨테이너의 fetch 는 시스템 루트만 신뢰하는 기본 클라이언트고(`session_mcp_tools.go` 의 `http.DefaultClient`), 그 컨테이너엔 사설 발급자를 넣을 자리가 없다 — credential-proxy 가 optional `ca-cert` 로 하는 일을 여기서는 할 수 없다. 차단 요인 `FETCH-ORIGIN` 이 그 판정과 해제 조건을 갖고 소관은 `tbm_session-platform-e2e-mock-policy` 다. 소유 파일이 사는 것은 **게이트까지**이고, 게이트를 통과했다는 결정 마커가 그 경계의 마지막 관측이다 ⚠️ **전제 정정(2026-09-13) — 이 행이 들고 있던 「도구는 https 만 받는데(`validateFetchTarget`)」는 #129(2026-09-12)로 거짓이 됐다**: `validateFetchTarget` 은 이제 `http` 와 `https` 를 **함께 받는다**(`url must be http or https` — 옛 리터럴 `url must be https` 는 코드 전수 0건이다). 그래서 **위의 CA 논증은 https 원본을 고를 때만 성립하고**, 검증할 인증서가 애초에 없는 평문 http 원본이 선택지에 들어왔다. 충실도 절의 같은 전제는 #133 이 이미 정정했고(218·243행) 이 행은 그 hunk 밖이라 남아 있던 사본이다. **이 정정은 해소 방향을 재판정하지 않는다** — 그 판정과 해제 조건은 위가 가리키는 `FETCH-ORIGIN` 행이 갖고, 소관도 그대로다. |
...
| 승인된 **실제 외부 호출로** 그 파일이 만들어지는 것 | `e2e_approval_gated_6_shared_volume_test.go` (대체 검증) | 위 두 행과 같은 차단 요인(`FETCH-ORIGIN`)이되 **막히는 자리가 다르다.** 스킴은 2026-09-12 에 열렸고(#129 — `validateFetchTarget` 이 `http` 와 `https` 를 함께 받는다), 그래서 위 두 행의 CA 논증이 https 원본을 고를 때만 성립하게 된 것은 **#133·#138 이 이미 그 행들에 정정으로 적었다**(2026-09-13). 이 행을 막는 것은 **크기**다 — `spillBody` 는 응답이 `maxInlineBodyBytes`(100,000)를 넘을 때만 돌고, 이 SUT 가 **무자격 GET 에 100,000 바이트 넘는 본문을 돌려주는 인클러스터 origin 을 하나도 세우지 않는다**. `deploy/kustomization.yaml` 을 전수로 읽어 확인했다: 세 대역(`e2e-anthropic-fake` · `e2e-approval-gateway-fake` · `e2e-k3s-mcp-fake`)이 GET 에 내는 것은 `/healthz` 와 게이트웨이의 작은 JSON 뿐이고(나머지 라우트는 전부 `POST`), **실물로 선 둘**도 마찬가지다 — `minio.yaml` 은 익명 읽기 정책을 세우지 않아(버킷 공개 설정이 없다) 서명 없는 GET 이 작은 403 XML 로 끝나고, `plugin-marketplace-git.yaml` 이 smart HTTP 로 내는 것은 `marketplace.json` 과 픽스처 SKILL.md 넷뿐인 저장소라 클론 전체가 그 선에 한참 못 미친다. ⚠️ 이 두 실물은 **대역이 아니라 원본**이므로, `FETCH-ORIGIN` 이 「정적 문서를 내는 평문 http 원본을 실물로 세운다」로 풀릴 때 **새로 세울 것 없이 큰 본문 하나만 더하면 되는 자리**이기도 하다 그래서 볼륨에 파일을 놓는 것은 fetch 가 아니라 테스트이고, 그 파일이 **제품이 쓰는 자리와 같은 자리**(`session_mcp_files.go` 의 `spillSubdir`·`spillFileSuffix`)라는 것까지가 산 것이다 — 막힌 것은 **누가 그 파일을 만들었는가** 한 조항뿐이다. 그런 origin 을 세우는 것은 대역 확장이고 소관은 `tbm_session-platform-e2e-mock-policy` 다 |
| (c) 동결·복원을 건넌 **파일 잔존** | 〃 | `approval-gated` 에 아카이브 전략이 없어 `POST /snapshot` 이 503 이라 **동결 진입 자체가 없다** — AC-F4 의 동결·복원 두 행과 **같은 선행**(AC-F5 아카이브 전략, 소관 `tbm_session-platform-docs-impl`)이다. 대체 검증이 사는 것은 **동결을 건너뛴** 왕복이고, 시나리오도 그 둘을 (b)·(c) 로 갈라 적는다 — 전자는 볼륨이 실어 나른다는 명제이고 후자는 아카이브가 실어 나른다는 명제다 |
| (d) `cursorBefore` 델타와 `offset=0` 의 **동결 전·후 전체** | 〃 | 〃 — 두 read 의 차이가 「동결 전·후」로 갈리는 것이 이 갈래의 내용이라, 동결이 없으면 남는 것은 커서 계약 일반이고 그쪽은 `state-api.md#시나리오 6` 의 대체 검증이 이미 소유한다(여기서 다시 사면 규칙 1 의 중복이다). 위 행과 같은 순간에 함께 열린다 |
