...
| 승인된 **실제 외부 호출로** 그 파일이 만들어지는 것 | `e2e_approval_gated_6_shared_volume_test.go` (대체 검증) | 위 두 행과 같은 차단 요인(`FETCH-ORIGIN`)이되 **막히는 자리가 다르다.** 스킴은 2026-09-12 에 열렸고(#129 — `validateFetchTarget` 이 평문 http 를 받는다) 위 두 행이 적는 「사설 인증서를 검증할 수 없다」도 그만큼 좁아졌다. 이 행을 막는 것은 **크기**다 — `spillBody` 는 응답이 `maxInlineBodyBytes`(100,000)를 넘을 때만 돌고, 이 배포에서 GET 에 답하는 인클러스터 origin 은 세 대역의 `/healthz` 와 게이트웨이의 작은 JSON 뿐이라 그 선을 넘는 본문이 없다. 그래서 볼륨에 파일을 놓는 것은 fetch 가 아니라 테스트이고, 그 파일이 **제품이 쓰는 자리와 같은 자리**(`session_mcp_files.go` 의 `spillSubdir`·`spillFileSuffix`)라는 것까지가 산 것이다 — 막힌 것은 **누가 그 파일을 만들었는가** 한 조항뿐이다. 그런 origin 을 세우는 것은 대역 확장이고 소관은 `tbm_session-platform-e2e-mock-policy` 다 |
