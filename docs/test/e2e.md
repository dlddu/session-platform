...
| 등재된 치환 `CLAUDE-PROVIDER` 의 **표현력 부족** — 치환이 없는 것이 아니라 **있는 치환이 모자라서** 경로에 진입조차 못 한다. 대역은 `content_block` 을 `text` 한 블록으로만 내고 `tool_use` 블록을 내지 못하므로 **모델이 도구를 부르는 갈래가 이 표면에서 시작되지 않는다**. 그래서 `approval-gated-workload.md#시나리오 3` 의 승인 왕복은 대역(`deploy/e2e-approval-gateway-fake.yaml`)과 제품 경로가 전 구간 실재하는데도 **진입 자체가 불가능**하다 — 도구 호출이 시작되지 않으면 승인 요청도 만들어지지 않는다. 등재↔코드는 1:1이라 R5 는 이것에 대해 영원히 침묵하고, 대역을 *더 넓게* 고쳐도 seam 의 **집합**은 바뀌지 않아 as-is 지문도 움직이지 않는다. 이 표가 아니면 어느 기계도 이 공백을 세지 않는다. | `deploy/e2e-anthropic-fake.yaml` | `content: [{ type: "text", text }]` | `확장` | `이 모델` | `없음` | 2026-09-25 | 대역에 `tool_use` 블록 발화를 더해(응답 성형 지시자처럼 **opt-in** 으로 — 기존 세 등재의 마커 단언을 흔들지 않는다) 이 행을 지우고, 같은 내용을 `CLAUDE-PROVIDER` 등재 행의 『잔여』에서 함께 덜어낸다. **신규 등재가 아니다** — 이미 등재된 EXT 치환의 표현력을 넓히는 형태라 판정이 `확장` 이고, 그래서 R10 은 이 행에 대해 「예고 CODE 가 아직 등재되지 않았을 것」이 아니라 **「넓힐 대상이 등재돼 있을 것」**을 검사한다. 리터럴은 대역이 낼 수 있는 content 가 text 한 블록뿐임을 코드로 증언하는 줄이므로, 확장되는 순간 사라져 게이트가 이 행을 물고 갱신을 요구한다. 풀리면 `#시나리오 3` 이 열리고 `#시나리오 6` 이 그것을 자기 선행으로 지목하므로 두 시나리오가 함께 착수 가능해진다 — 저작은 `tbm_session-platform-scenario-e2e` 소관이고, 이 행이 사는 동안 그 표의 「선행: 없음」은 `#시나리오 3` 에 대해 거짓이다. |
<!-- /fidelity:blockers -->
...
- 차단 요인 **2**건 (seam이 아니다 — 승인 뒤의 아웃바운드 호출, 그리고 등재된 공급자 대역이 `tool_use` 를 못 내 승인 왕복에 진입조차 못 하는 자리)
- 착수 가능 **1**건 — `선행: 없음` + `소관: 이 모델` 인 행. 선행이 없으니 이 모델의 다음 감지가 작업을 연다
<!-- /fidelity:summary -->
...
| `claude-code-workload.md#시나리오 3` | go | **없음** | 직렬 큐는 실재하고(`data-plane/cmd/agent/claude.go`의 프롬프트 큐), 관측 수단에도 선례가 있다 — `control-plane/test/e2e_e2_prompt_invocation_test.go`가 프로세스 테이블에서 invocation argv를 세는 프로브를 이미 세웠다(자기 프로세스와 flag를 언급만 하는 프로세스를 거르는 두 가드까지). 이 파일이 살 것: 두 실행 구간의 **비중첩**(동시 argv 수가 1을 넘지 않음)과 read(`offset=0`) 누적 순서 = write 순서. 시나리오 2와의 경계: 2는 「write 1회 = 1 실행」, 3은 **두 write 사이의 순서**. ⚠️ **2026-09-10 실측(1차)** — 기대 결과의 뒷절반은 당시 SUT 에서 **귀속할 표지가 없었다**: 공급자 대역이 프롬프트와 무관하게 같은 상수 한 줄을 돌려주므로 출력 버퍼의 두 응답이 **바이트 동일**이었다. ✅ **2026-09-10 해소(2차)** — `claude-code-workload.md#시나리오 4` 저작 슬라이스가 대역을 넓혀 프롬프트의 지시자가 응답의 **라벨과 크기**를 고르게 했다(`deploy/e2e-anthropic-fake.yaml`). 서로 다른 라벨을 쓰면 두 응답이 버퍼에서 갈리므로 뒷절반은 이제 관측 가능하다 — 이 행이 남아 있는 이유는 **파일이 아직 없다**는 것 하나뿐이다. 앞절반(비중첩)은 시나리오 2 의 파일이 **이미** 같은 프로브로 산다(동시 argv 수가 1을 넘으면 실패)는 점은 그대로이므로, 이 파일의 산출은 **순서** 쪽에 둘 것. |
