...
삼는데 kind SUT가 그 세션을 띄우지 못하기 때문이다. 원인은 하나가 아니라 셋이고 순서가 있다 —
아래 「차단 요인」에 원장으로 있다. 여기에 사본을 두지 않는다(그 표가 정본이다).
...
| --- | --- | --- | --- |
| `web/e2e/journeys/j6-agent-prompt-loop.spec.ts` | `.route(` | `installAgentApi`가 12개 테스트 전부에 `/api/v1/**` 핸들러를 설치해 세션·config·SSE 스트림을 위조한다. | 아래 「차단 요인」 3행을 그 표의 '해소 시' 칸대로 해소(등재 2 + 실배포 1)한 뒤, **claude-code 세션을 대상으로 유지한 채** 실 SUT 위에서 재작성. 순서가 있다 — ①② 가 서지 않으면 pod가 Ready에 못 가 세션 자체를 만들 수 없고, 그러면 ③ 만 고쳐도 소용이 없다. |
...
**판정은 미리 확정한다**(등재는 아니다 — 대체물이 서기 전에는 "e2e에서 실제로 구동되는 구간"
칸에 쓸 것이 없어 등재가 거짓이 된다). 판정 기준은 정의의 `EXT` 배제 조항 하나다: *"kind에
...
| --- | --- | --- | --- | --- |
| ① claude-code 세션 pod가 Ready에 못 간다: `entrypoint.sh`가 agent를 exec하기 전에 K3s MCP를 호출해 단명 GitHub token을 받는다. `set -eu` + `curl --fail`이라 실패가 곧 컨테이너 종료이고, kind가 넣는 토큰은 placeholder다. | `data-plane/entrypoint.sh` | `${K3S_MCP_TOKEN:?` | `EXT` | 그 MCP는 **실 GitHub App 개인키를 쥔 조직 서비스**라 인클러스터에 기능하는 실물을 세울 수 없다(복제해도 자격이 없어 토큰을 못 만든다) → EXT 자격을 만족한다. 인클러스터 대체 발급 경로를 세우고 `PLUGIN-CRED`(EXT)로 등재. 잔여는 "프로덕션의 실제 자격 발급 왕복은 미검증". |
| ② 그 토큰으로 **비공개** `dlddu/plugin-marketplace`에서 플러그인을 설치한다. URL이 entrypoint에 상수로 박혀 있어 갈아끼울 수 없다. | `data-plane/entrypoint.sh` | `https://github.com/dlddu/plugin-marketplace.git` | `없음` | marketplace는 **git 원격일 뿐**이고 `claude plugin marketplace add <url>`는 호스트를 가리지 않는다 — MinIO가 S3를 실물로 세웠듯 인클러스터 git 원격을 세우면 **같은 코드 경로**가 그대로 돈다. 실물 배포가 가능하므로 EXT를 쓸 수 없다: URL을 설정값으로 빼고 인클러스터 원격을 배포해 **등재 없이** 해소한다. |
| ③ 제공자에 도달할 수 없다: ①②가 풀려도 `claude-code-credentials`의 base-url이 의도적으로 unroutable이라 실 세션이 어떤 응답도 받지 못한다. | `deploy/claude-code-credentials-secret.yaml` | `https://127.0.0.1:9` | `EXT` | Anthropic API는 클러스터에 존재시킬 수 없다(MinIO를 EXT에서 뺀 근거가 여기엔 해당하지 않는다) → EXT 자격을 만족한다. 결정적 응답을 내는 인클러스터 가짜 provider를 `deploy/`에 배포하고 base-url을 그리로 돌린 뒤 `CLAUDE-PROVIDER`(EXT)로 등재. 잔여는 "실 provider의 스트리밍·에러 계약은 미검증". |
<!-- /fidelity:blockers -->
...
- 차단 요인 **3**건 (seam이 아니다 — 판정만 확정, 등재 0)
<!-- /fidelity:summary -->
