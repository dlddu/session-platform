...
공백을 메우는 것이 대역이 아니라 **실물 배포**이기 때문이다: 정적 문서를 내는 http 원본은
MinIO · 마켓플레이스 원격과 같은 성격이라 kind 에 실제로 세울 수 있고, 그러면 정의의 EXT 배제
조항이 그대로 걸린다. **그 원본의 스킴은 2026-09-13 재판정에서 평문 http 로 정해졌다** — #129 이후 도구가 `http` 를 함께 받으므로 검증할 인증서가 없는 원본이 가능해졌고, 그것이 이 행의 마지막 `선행` 이던 「해소 방향 재판정」을 닫았다.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 승인된 `web_fetch_get` 의 **실제 아웃바운드 호출**. 그 호출을 받아 줄 upstream 이 이 SUT 에 아직 **없어서** 승인 갈래는 *게이트를 통과했다* 는 데까지만 관측된다 — `e2e_approval_gated_3_approval_path_test.go` 가 사는 경계가 거기다. ✅ **2026-09-13 — 이 행의 마지막 `선행` 이던 「해소 방향 재판정」을 여기서 끝냈다.** 오래 이 행을 세워 둔 벽은 「인클러스터 원본은 https 여야 하는데 그 인증서를 검증할 길이 없다」였다: MCP 컨테이너의 fetch 는 시스템 루트만 신뢰하는 기본 클라이언트이고, credential-proxy 가 optional `ca-cert` 로 하는 일을 여기서는 할 수 없다(`helperPodSpec` 이 넣는 env 는 워크로드·주소·세션ID·게이트웨이 3종뿐이라 CA 를 넣을 자리가 아예 없다). **그 벽은 전제와 함께 무너졌다** — #129 로 `validateFetchTarget` 이 `http` 와 `https` 를 함께 받게 되면서 「인클러스터 원본은 https 여야 한다」는 고리가 끊겼고, **평문 http 원본에는 검증할 인증서가 애초에 없다.** CA 를 넣을 자리가 없다는 사실은 그대로지만 더 이상 아무것도 막지 않는다. 그 컨테이너가 평문 http 인클러스터 주소로 나가는 것도 가설이 아니다 — **이 스위트가 매 실행 이미 하고 있다**(같은 `session-mcp` 컨테이너가 `$APPROVAL_GATEWAY_URL` = `http://approval-gateway-fake:8080` 으로 요청을 낸다). 남은 것은 그 upstream 을 세우는 일 하나이고 제품 변경을 요구하지 않는다 ⇒ `선행` 은 「없음」이다. | `data-plane/cmd/agent/session_mcp_tools.go` | `http.DefaultClient.Do(req)` | `없음` | `이 모델` | 없음 | 2026-10-11 | **해소 방향 확정(2026-09-13) — 정적 문서를 내는 평문 http 원본을 인클러스터에 실물로 배포해 등재 없이 닫고 이 행을 지운다.** 그런 원본은 kind 에 그대로 세울 수 있으므로 정의의 EXT 배제 조항이 걸린다(MinIO · `deploy/plugin-marketplace-git.yaml` 선례) — 세울 실물이 없어 EXT 로 간 게이트웨이·공급자 대역과 성격이 다르다. 호출 관찰도 치환을 들이지 않는다: 원본 자신의 액세스 로그가 그 GET 을 센다. 그래서 예고할 등재 CODE 가 없고, 해소되면 이 행은 남기지 않고 **지운다**. 앞서 열려 있던 셋은 이렇게 갈렸다 — ① 공개 https 원본을 실물로: 클러스터 밖 네트워크에 의존해 재현성이 떨어진다 ② 대역 + CA 주입 재검토: 제품에 CA 주입을 스테이징한 PR #93 의 반려 근거는 여전히 유효하고, 평문 http 로 가면 검증할 인증서 자체가 없어 그 경로가 필요 없어진다 ③ 관측 지점을 세션 런타임 밖으로: 소관이 다른 모델로 넘어가는데 여기서 닫을 수 있으므로 택하지 않는다. **착수 슬라이스가 할 일**: 정적 문서를 내는 평문 http Deployment·Service 를 `deploy/` 에 더해 `deploy/kustomization.yaml` 에 엮고 → `e2e_approval_gated_3_approval_path_test.go` 의 `f3Prompt` 대상 URL 을 그 주소로 바꾸고 → 「승인 후 GET 이 정확히 1회」와 「결정 전 무도달」 두 단언을 세우고 → §「남은 미검증 분기」의 그 두 행과 이 행을 함께 지운다. 그 슬라이스가 착지해도 리터럴 `http.DefaultClient.Do(req)` 는 코드에 그대로 남지만, 이 행이 사라지므로 대조 대상도 함께 없어진다. |
<!-- /fidelity:blockers -->
...
- 착수 가능 **1**건 — `선행: 없음` + `소관: 이 모델` 인 행. 2026-09-13 에 그 1건의 「해소 방향 재판정」이 끝나 `선행` 이 「없음」이 됐다 — 다음 감지가 착수 슬라이스를 연다
<!-- /fidelity:summary -->
