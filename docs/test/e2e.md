...
그대로, 새로 발견되는 "치환조차 없는 공백"의 원인은 여기에 다시 쌓인다. **2026-09-07 현재
비어 있다.** 마지막 행이던 approval-gated 세션의 **결재 이후** 구간은 원인이 사라져 나갔다 —
승인된 `web_fetch_get` 의 호출을 내는 MCP 컨테이너가 사설 발급자를 신뢰할 길이 아예 없었는데,
게이트웨이 Secret 의 optional `ca-cert` 키(`SESSION_MCP_CA_CERT`)와 그 PEM 을 시스템 풀에
**추가한** fetch 클라이언트가 그 길을 냈다. 남은 것은 「치환조차 없다」가 아니라 아직 쓰이지
않은 e2e 다: 정적 문서를 내는 https 원본을 kind 에 **실물로** 세우고(MinIO · 마켓플레이스 원격과
같은 성격이라 등재 없이 닫힌다) 그 위에서 승인 왕복을 도는 AC-F3 전용 파일을 쓰면 된다 — 전자는
`deploy/`, 후자는 아래 `ac-summary` 의 공백 회계가 소유한다.
...
- 차단 요인 **0**건 (seam이 아니다 — 먼저 있던 넷이 모두 등재·실배포·제품 변경으로 나갔다)
<!-- /fidelity:summary -->
