...
- **구현**: `control-plane/test/integration_test.go`의 `TestScenario4_CRIUIntegrity`는 env/cluster가 준비된 경우 실행하고, kind SUT에서는 `control-plane/test/e2e_d4_process_tree_test.go`의 `TestProcessTree_EnvAndCwdSurviveTheFreeze`가 제품 snapshot endpoint를 통해 동결→복원을 건넌 환경 변수·작업 디렉터리를 단언한다(커서 연속성 절반은 AC-B3의 `e2e_b3_restore_integrity_test.go`가 소유한다). CRIU·MinIO를 켠 오버레이에서 실제 dump→pod 회수→새 pod restore가 일어나는지는 `../criu-verification.md`가 다룬다
...
> 비고: 바쁜 포그라운드 작업 중 유휴 도달 시의 스냅샷 트리거는 AC-B1의 트리거 정책 확정 후 별도 시나리오로 추가한다(`../doc-tracker/`의 열린 항목 참고).
