...
> 모든 가치가 최소 1개 여정에 연결되어 **고아 여정(가치 없는 여정) 없음**. J5는 2026-08-08 V6 삭제에 따라 V3(연속성)으로 재연결됨.
> mockup 매핑 결과 **시각화 없는 가치 없음**(V1~V5 전부 1개 이상 mockup 연결). 단계 단위로는 J1 완전·J2/J3/J5 부분·J4 의도적 비시각화이며, `J2-S3`·`J3-S4`·`J5-S4`가 미시각화로 남아 있습니다. J5 단계(S1~S3)는 `workspace.html` session shell 콘솔에 쉘 명령 입력·stdout 출력으로 시각화됩니다(2026-07-01 콘솔을 key/value→쉘로 갱신). (단계별 매핑: `../mockups/README.md` · 상세: `../doc-structure-state.md`)
