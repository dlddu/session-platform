...
> 각 여정 단계는 mockup으로 시각화됩니다. 현재 mockup 4종(`../mockups/`)이 작성되어 **J1은 완전·J2/J3/J5는 부분** 시각화,
...
| V1 세션 격리 | J1 | ✅ 연결됨 |
| V2 유휴 자원 회수 | J2 | ✅ 연결됨 |
| V3 끊김 없는 세션 연속성 | J2, J3, J4, J5 | ✅ 연결됨 |
| V4 자유로운 멀티세션 전환 | J3 | ✅ 연결됨 |
| V5 일관된 세션 상태 | J1, J4 | ✅ 연결됨 |
| V8 목적에 맞는 작업 환경 선택 | — | ❌ 여정 없음 |

> **고아 여정(가치 없는 여정)은 없으나**, 2026-08-08 신설된 V8(작업 환경 선택)에 대응하는 여정이 없다 — 워크로드 타입을 고르는 흐름(`../prd/claude-code-workload.md` AC-E1)을 다루는 여정 신설이 필요하다. J5는 같은 날 V6 삭제에 따라 V3(연속성)으로 재연결됨.
> mockup 매핑 결과 **V8은 시각화 없음**(V1~V5는 1개 이상 mockup 연결). 단계 단위로는 J1 완전·J2/J3/J5 부분·J4 의도적 비시각화이며, `J2-S3`·`J3-S4`·`J5-S4`가 미시각화로 남아 있습니다. J5 단계(S1~S3)는 `workspace.html` session shell 콘솔에 쉘 명령 입력·stdout 출력으로 시각화됩니다(2026-07-01 콘솔을 key/value→쉘로 갱신). (단계별 매핑: `../mockups/README.md` · 상세: `../doc-structure-state.md`)
...
- ~~**"세션"의 정체**~~ → **2026-07-01 확정**: 세션 = 전용 pod의 인터랙티브 쉘(`../prd/shell-workload.md`). **2026-08-08 개정**: 워크로드 타입이 `shell`·`claude-code` 둘로 늘어남(`../prd/claude-code-workload.md`) — 여정은 아직 `shell` 타입만 다루므로 `claude-code` 여정 신설 여부 검토 필요. 페르소나·여정 표현을 "인터랙티브 쉘" 기준으로 재검토 가능. **제품명·소유자**는 여전히 임시값(`../doc-tracker.md` 참고).
