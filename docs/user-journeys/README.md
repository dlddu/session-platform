...
이 폴더는 가치(V1~V6)를 사용자 흐름으로 푸는 **사용자 여정 문서**입니다. 여정별로 파일이 나뉘어 있고,
...
> 세션은 **전용 pod에서 실행되는 인터랙티브 쉘**입니다(가치 V6 · `../prd/shell-workload.md`).
...
| J5 | 쉘에서 명령을 실행하고 상태를 이어간다 | P1 | V6, V3 | [j5-shell-interaction.md](./j5-shell-interaction.md) |
...
| V6 인터랙티브 쉘 세션 | J5 | ✅ 연결됨 |

> 모든 가치가 최소 1개 여정에 연결되어 **고아 여정(가치 없는 여정) 없음**. V6는 J5 신설로 연결됨.
> mockup 매핑 결과 **시각화 없는 가치 없음**(V1~V6 전부 1개 이상 mockup 연결). 단계 단위로는 J1 완전·J2/J3/J5 부분·J4 의도적 비시각화이며, `J2-S3`·`J3-S4`·`J5-S4`가 미시각화로 남아 있습니다. J5 단계(S1~S3)는 `workspace.html` session shell 콘솔에 쉘 명령 입력·stdout 출력으로 시각화됩니다(2026-07-01 콘솔을 key/value→쉘로 갱신). (단계별 매핑: `../mockups/README.md` · 상세: `../doc-structure-state.md`)
...
- ~~**"세션"의 정체**~~ → **2026-07-01 확정**: 세션 = 전용 pod의 인터랙티브 쉘(V6 · `../prd/shell-workload.md`). 페르소나·여정 표현을 "인터랙티브 쉘" 기준으로 재검토 가능. **제품명·소유자**는 여전히 임시값(`../doc-tracker.md` 참고).
