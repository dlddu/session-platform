...
- **claude-code 모델 정책 (AC-E6)** — *2026-08-08 확정*. model은 생성 시 불변이며, 미지정은 특정 공급자 버전을 고정하지 않는 `platform-default` 별칭으로 저장한다. 별칭은 CLI `--model`을 생략하고 명시 model만 단일 argv로 전달한다.
...
> ✅ mockup의 `model-a`/`model-b`는 계속 예시일 뿐이며 제품 UI는 자유 입력 + `platform-default`를 사용한다. 대화 재개 표현은 첫 성공 실행 이후 `--continue` 결정으로 구체화되었다. 시각 정본은 `web/src/` 코드다.
...
- **구현 사슬 반영 (AC-E1~E6)** — *2026-08-08 갱신*: OpenAPI/도메인/model/workload pod 분기, **HTTPS-only** credential-proxy sidecar에 한정된 SecretKeyRef, multi-workload data-plane 이미지, serial Claude runner, 첫 성공 이후 `--continue`와 고정 HOME/workdir, prompt 1 MiB/413, invocation 16 MiB·누적 256 MiB output marker/507, cursor와 output-full 상태를 보존하는 CRIU 비대상 archive restore, 타입별 SPA picker/route/workspace가 구현되었다. SPA의 `submissions · checking output`은 클라이언트 polling 상태이고 서버 running/queued 상태는 아니다. archive 외부 전송은 `CLAUDE_CODE_ARCHIVE_ENABLED` opt-in(기본 off)이다. fake runner·단위/통합·Playwright route fixture로 결정적 계약을 검증하며, 실제 외부 Claude API 배포 smoke는 아직 별도 opt-in 범위다.
