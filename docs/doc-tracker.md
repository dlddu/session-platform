...
- **🆕 plugin 부트스트랩과 egress 허용 목록의 충돌 (AC-F6 ↔ AC-F2)**: AC-E6의 런타임 plugin 부트스트랩은 주 컨테이너가 K3s MCP와 `github.com`으로 **직접 나가는 것**을 전제하는데, AC-F2의 허용 목록에는 그 두 목적지가 없다. 선택지는 ① 이 타입에서는 런타임 부트스트랩과 `k3s-mcp-token`을 두지 않고 플랫폼 도구를 세션 MCP 파드로만 노출, ② MCP 파드가 plugin seed를 공유 볼륨(AC-F5)에 내려놓고 주 컨테이너가 읽기, ③ 두 목적지를 허용 목록에 추가(비권장). **현재 AC-F6은 ①을 전제로 기술되어 있다** — 즉 이 타입의 주 컨테이너에는 `k3s-mcp-token`이 없다. ②를 고르면 기동 순서 의존이 생기고, ③은 AC-F2를 다시 느슨하게 만든다.
