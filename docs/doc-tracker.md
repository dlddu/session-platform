...
- **🆕 세션 레코드의 pod 참조가 단수 (AC-A2 개정 ↔ AC-F4)**: AC-A2가 "워크로드 파드 1:1 + 보조 파드"로 개정되었으나, 구현의 세션 레코드는 `session.go:198`의 `Pod string` **단수 문자열**이다. 따라서 보조 파드는 필드 하나를 더 붙이는 문제가 아니라 **참조 모델 자체를 바꾸는** 일이다 — 조회/스냅샷/복원/삭제가 모두 "그 세션의 pod 집합"을 다뤄야 하고, `PodOrchestrator.Start`/`RestoreInto`/`Stop`과 e2e의 `pod` 필드(`e2e_test.go:49`), 라벨 기반 조회가 함께 바뀐다. `approval-gated` 구현의 첫 관문이며, 이 항목이 그 부채의 단일 관리 지점이다.
