...
- **🆕 사이드카 egress와 파드 단위 정책의 간극 (AC-F2 ↔ AC-F6)**: NetworkPolicy는 **파드 단위**로 적용되므로, credential-proxy 사이드카가 공급자 HTTPS로 나가야 하는 한 같은 파드의 주 컨테이너도 L3/L4에서는 같은 목적지에 닿는다. 즉 `approval-gated`의 승인 게이트에 구멍이 하나 남아 있다. 선택지는 ① 공급자 프록시를 사이드카에서 **보조 파드로** 이관(참조 구현 `pure-agent`의 llm-gateway 방식)하여 허용 목록을 kube-dns + 세션 MCP로만 좁히거나, ② 수용된 예외로 명시하고 남기는 것. **현재 AC-F2는 ②를 전제로 기술되어 있다.** ①을 고르면 AC-F2·AC-F4·AC-F6이 함께 개정된다.
