...
- **🆕 세션당 파드 수 증가 (AC-F4)**: `approval-gated`는 세션마다 파드가 셋(워크로드 + MCP + 프록시)이다. 동결된 세션은 셋 다 회수되므로 유휴 점유는 0이지만, **활성 세션당 스케줄링 단위와 기동 지연이 늘어난다.** 이는 AC-F2의 egress 경계를 파드 단위 정책으로 강제하기 위한 비용이며, 용량 계획이 붙을 때 재검토 대상이다.
...
- **공급자 프록시의 배치 (AC-F2 ↔ AC-F4/F6)** — *2026-09-03 확정*. 초판은 credential-proxy를 `claude-code`(AC-E6)처럼 워크로드 파드 안의 **사이드카**로 두었고, NetworkPolicy가 파드 단위라 공급자 HTTPS origin을 허용할 수밖에 없어 주 컨테이너도 같은 목적지에 닿는 구멍이 남았다. 이를 **프록시를 보조 파드로 분리**하는 쪽(참조 구현 `pure-agent`의 llm-gateway 배치)으로 확정했다 — 이제 워크로드 파드의 egress 허용 목록에 **외부 origin이 하나도 없다.** 프록시의 **동작 계약 정본은 계속 AC-E6**이고 배치와 바인딩(loopback → 파드 IP + ingress 제한)만 타입별로 다르다. 대가는 위 🟡의 세 항목(평문 홉·plugin 부트스트랩·파드 수)으로 등재했다.
