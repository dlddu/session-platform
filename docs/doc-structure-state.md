...
- **마지막 검증 시점**: 2026-08-08 (V8·`claude-code` 미시각화 위험 해소 — J6 신설 + mockup 확충)
- **검증 범위**: 상류에서 확정된 `claude-code` 워크로드 타입(`prd/claude-code-workload.md` AC-E1~E6)과 가치 V8에 대응하는 여정·mockup을 신설하고 전 매트릭스를 재계산. 함께 J5-S4 미시각화와 `restore.html` 콘솔 미갱신(직전 판의 🟡 위험 2건)도 해소. 잔여: J2-S3·J3-S4 미시각화, 디자인 시스템 부재
...
- **건강 상태**: 🟢 **가치 측 위험 0 · 시각화 없는 가치 0** — V1~V5·V8 전부 여정과 mockup에 연결됨. J1·J5·J6은 완전 시각화, J2·J3는 부분(각 1단계씩 ❌), J4는 백엔드 여정이라 의도적 비시각화. 남은 위험은 🟡 미시각화 2단계와 🟢 디자인 시스템 부재(mockup 5종 전부 임의 스타일).
...
### mockup ↔ 디자인 시스템
- 디자인 시스템 미정의 → index/new-session/workspace/agent-workspace/restore **5개 전부 임의 스타일 mockup(🟢)**. 인라인 CSS 변수 사용.
- `agent-workspace.html`은 `workspace.html`의 인라인 토큰을 **복사**해 만들었다. 겉보기 일관성은 있으나 시스템에 연결된 것이 아니므로, 토큰이 바뀌면 5개 파일을 각각 고쳐야 한다 — 디자인 시스템 부재의 비용이 파일 수만큼 늘어난 상태.
...
### 🟢 디자인 시스템 측 위험
- **임의 스타일 mockup**: **5개 전부**(4→5로 1개 증가). 디자인 시스템 부재로 인라인 토큰 사용. 디자인 시스템 셋업(진입점 5) 후 마이그레이션 또는 시스템 편입 필요.
- **주입형 스킬 미설치**: `.claude/skills/`에 `ui-with-design-system`·`screen-with-mockup-and-design-system` 없음. 디자인 시스템이 아직 없어 지금은 실익이 낮으나, 셋업 시 함께 검토 대상.
...
| 🟢 | 디자인 시스템 부재 / 임의 스타일 5 mockup | 디자인 시스템 셋업(토큰/컴포넌트/패턴) + `.claude/skills/` 주입형 스킬 추가 검토(진입점 5), 이후 mockup 마이그레이션. **mockup이 5종으로 늘어 마이그레이션 비용이 커지는 중** |
