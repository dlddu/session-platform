...
| 🔵 | model catalog 예시 이름 | mockup을 실제 runtime config에 연결하면 `model-a`/`model-b` 고정 예시 제거 |
...
| 2026-08-29 (6) | **여정 문서 6종 형식 정규화** — J1~J6을 `user-journey-writer` 템플릿의 9개 섹션(문서 정보·여정 정의·단계별 상세·분기·예외·측정 지표·변경 이력·인접 여정·구현 상태)으로 통일하고, 23개 단계 전부에 사용자 행동·터치포인트·생각·감정·페인포인트를 채웠다. **여정 식별자·단계 식별자·AC 참조·mockup 매핑은 전부 불변**이므로 가치/여정/mockup 매트릭스(✅14 ⚠️4 ❌2 ⚪3, 23단계)는 **변화 없음**. 형식 규약은 `user-journeys/README.md`에 기록. | 여정 6종의 섹션 구성이 제각각(J1~J4는 단계만, J5는 +인접 여정, J6은 +구현 상태) | 6종 모두 동일한 9개 섹션 |
