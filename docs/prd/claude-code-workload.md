...
> 직접 나가야 하는데 `approval-gated`의 egress 허용 목록에 그 목적지가 없다. 2026-09-03에 그 타입에서는
> **부트스트랩을 아예 두지 않는 것**으로 확정했다(AC-F6). 따라서 이 문서의 plugin 배선을 고쳐도
> `approval-gated`는 영향을 받지 않으며, 반대로 **plugin이 제공하는 플랫폼 도구를 그 타입도 써야 한다면
> 세션 MCP 쪽에 따로 실어야 한다.**
