...
- **V1 세션 격리** — 에이전트 실행과 자격 증명을 세션 전용 워크로드 파드에 가둠 (AC-A1/A2 구체화). credential-proxy는 같은 파드 안의 사이드카 컨테이너이므로 AC-A2의 보조 파드가 아니다
...
>
> ⚠️ **다만 AC-E1(타입 선택)은 예외다.** 이 AC는 상위 AC의 구체화가 아니라 새 축이라 상속할 가치가 없다. 2026-08-08 3차 개정에서 이를 뒷받침할 **V8(목적에 맞는 작업 환경 선택)** 이 신설되어 AC-E1 → V8로 직접 연결되었다(부가로 V1: 타입이 달라도 1세션=1전용 워크로드 파드 매핑 유지).

> 이 PRD는 세션 워크로드가 **단수(인터랙티브 쉘)에서 복수(타입)로 확장**되는 변경을 다룬다.
> AC-E1이 타입 선택이라는 새 축을 도입하고, AC-E2~E6은 `claude-code` 타입에서 기존 AC들
> (AC-A1·B1·B2·B3·C2·C3)이 어떻게 구체화되는지를 못박는다. 다른 타입의 대응 명세는
> `shell-workload.md`(AC-D1~D5)와 `approval-gated-workload.md`(AC-F1~F6)이며, 세 문서는 같은 상위 AC에
...
> 파드 안의 loopback 사이드카이고, `approval-gated`는 같은 계약의 프록시를 **세션 헬퍼 보조 파드의 컨테이너**로
> 옮겨 파드 IP에 바인딩한다(AC-F2·F4·F6). 동작 계약이 갈라지지 않도록 **정본은 계속 AC-E6**이며 배치만 타입별로 다르다.
> ② 주 컨테이너의 `k3s-mcp-token`과 **런타임 plugin 부트스트랩의 위치** — 부트스트랩이 K3s MCP·`github.com`으로
> 직접 나가야 하는데 `approval-gated`의 egress 허용 목록에 그 목적지가 없다. 2026-09-08에 그 타입에서는
> **헬퍼 파드가 skills-only seed를 공유 볼륨에 내려놓고 워크로드 파드가 그 로컬 경로에서 설치하는 것**으로
> 확정했다(AC-F6). 따라서 **같은 marketplace의 같은 plugin이 두 타입에 서지만**, 이 문서가 기술하는 것은
> 주 컨테이너가 직접 마켓플레이스에 나가는 배치뿐이다. 이 문서의 plugin 배선을 고치면 `approval-gated`의
> seed에도 그대로 반영되지만, **plugin이 물고 오는 MCP 서버는 그 타입에서 seed 단계에 제거된다** — 승인
> 게이트를 지나지 않는 도구 표면이기 때문이다(AC-F6).

## Acceptance Criteria

### AC-E1: 세션 워크로드 타입 선택
- **설명**: 세션 생성 요청은 워크로드 타입 `workloadType`을 받는다. 허용값은 `shell`·`claude-code`·`approval-gated`(AC-F1)이며, **필드 생략만** `shell` 기본값으로 해석한다(explicit empty/null/비문자열은 400). 타입은 세션 수명 동안 **불변**이다(타입을 바꾸려면 새 세션을 만든다). 하나의 세션은 정확히 하나의 워크로드 타입을 가지고, control plane은 타입에 따라 서로 다른 data plane 워크로드(에이전트 모드/이미지)로 pod를 프로비저닝한다. 타입이 늘어나도 AC-A1(control plane은 워크로드를 직접 실행하지 않음)과 AC-A2(1 세션 = 1 전용 워크로드 파드)는 그대로 유지된다.
- **달성 가치**: V8, V1
...
### AC-E6: 공급자 자격 증명은 sidecar Secret, 플러그인 토큰은 main Secret, 플랫폼 모델은 optional Secret
- **설명**: 플랫폼 전역 Kubernetes Secret의 **필수** `base-url`·`auth-token` 키는 각각 `ANTHROPIC_BASE_URL`·`ANTHROPIC_AUTH_TOKEN`으로 세션 pod의 localhost credential-proxy sidecar에만 주입된다. Claude CLI·Bash가 실행되는 주 컨테이너에는 실제 공급자 자격 증명을 주입하지 않고, localhost URL과 비밀이 아닌 proxy placeholder만 준다. 세션 pod에는 전용 `data-plane` ServiceAccount token을 마운트하며, Kubernetes 기본 `view` ClusterRole을 클러스터 범위로 바인딩해 Secret을 제외한 읽기 권한만 제공한다. 두 컨테이너는 네트워크만 공유하고 PID·파일시스템은 공유하지 않는다. proxy는 시작 시 **HTTPS** upstream origin을 고정하고, 허용 목록에 든 Anthropic/Claude/Stainless 요청 헤더만 전달한 뒤 플랫폼 토큰 하나를 주입한다. CONNECT·Upgrade와 모든 요청 trailer를 거부/제거하고, 중간 1xx를 포함한 응답에서 토큰 리터럴이 클라이언트로 노출되지 않게 한다. Provider SSE response는 raw upstream 64 MiB 상한 내에서 whole-body buffering 없이 증분 전달하여 안전한 upstream flush가 EOF 전에 Claude CLI에 도달한다. proxy의 tail-safe redactor는 token 후보 suffix를 response read 경계 너머까지 보류하므로 split credential도 일시적으로 노출하지 않는다. 자격 증명은 세션 생성 요청으로 지정할 수 없고, 세션 조회 응답·로그·read·SSE 출력 어디에도 토큰 값이 노출되지 않는다. runner 출력의 별도 증분 redactor도 token이 여러 runner write에 걸쳐도 가능한 match suffix를 보류한다. 별도의 optional `ca-cert` 키는 자격 증명이 아니라 **신뢰 앵커**다 — CA 인증서는 공개 물건이므로 주소류 설정에 가깝다. 공급자 `base-url`이 공개 저장소가 모르는 발급자의 사설 게이트웨이일 때만 필요하며, 자기가 짝지어진 자격 증명과 같은 **credential-proxy 컨테이너에만** `ANTHROPIC_CA_CERT`로 optional SecretKeyRef 주입되고 주 컨테이너에는 노출되지 않는다. proxy는 이 PEM 번들을 시스템 루트 풀에 **추가**할 뿐 대체하지 않으므로, 공개 발급 엔드포인트를 쓰는 배포도 이 키가 있는 채로 계속 동작한다. 키가 없거나 값이 비면 지금까지처럼 시스템 풀만 쓰고 `k8s/` 매니페스트는 한 줄도 달라지지 않는다. 값이 비어 있지 않은데 인증서가 하나도 파싱되지 않으면 첫 요청까지 미루지 않고 proxy 시작을 실패시킨다 — 조용히 시스템 신뢰로 되돌아가면 그 키를 넣은 이유였던 게이트웨이로의 모든 호출이 거부되기 때문이다. 이 키는 신뢰하는 발급자 집합만 넓히며, upstream origin을 시작 시 **HTTPS**로 고정하는 요구를 낮추지 않는다. 반면 Secret의 `model` 키는 자격 증명이 아닌 **optional 플랫폼 기본값**이다. 세션 metadata가 `platform-default`일 때만 주 컨테이너의 `CLAUDE_CODE_MODEL`에 optional SecretKeyRef로 주입되고 sidecar에는 노출되지 않는다. 키가 없거나 값이 비면 Claude CLI의 기존 기본 선택을 쓰도록 `--model`을 생략한다. 생성 요청의 `model` 필드에 구체 값을 명시하면 SecretKeyRef 대신 그 값을 literal `CLAUDE_CODE_MODEL`로 주입하여 플랫폼 기본값보다 우선한다. model 필드의 **생략만** 안정적인 별칭 `platform-default`로 해석하며 explicit empty/null/비문자열, 앞뒤 공백이 있는 값, 허용 pattern 밖의 값은 pod 생성 전에 400으로 거부한다. 세션에 저장된 model 또는 별칭은 수명 동안 불변이다(다른 설정을 고르려면 새 세션을 만든다 — AC-E1의 타입 불변성과 같은 규칙).

  Data plane의 `entrypoint.sh`는 `claude-code` workload에서만 agent 시작 전에 `K3S_MCP_TOKEN`으로 K3s MCP의 `github_app_installation_token`을 호출한다. 발급 범위는 private `plugin-marketplace` 저장소의 `contents:read`로 제한하며, 단명 GitHub token은 해당 저장소 URL에 한정한 프로세스 범위 Git HTTPS Authorization header로만 전달한 채 `claude plugin marketplace add https://github.com/dlddu/plugin-marketplace.git`와 `claude plugin install session-platform@dlddu-plugins`를 실행한다. 이후 token과 header를 unset하고 runtime plugin seed 경로만 agent에 상속한다. shell과 credential-proxy는 이 bootstrap을 건너뛴다. 세션 HOME의 managed settings는 이 plugin만 활성화한다. 플랫폼 Secret의 **필수** `k3s-mcp-token` 키는 `K3S_MCP_TOKEN`으로 주 컨테이너에만 주입하며 credential-proxy와 control plane에는 주입하지 않는다. plugin MCP는 이 환경 변수를 사용하고, agent는 그 literal을 runner 출력의 증분 redaction 대상에 포함한다. 부트스트랩이 닿는 **두 상류 주소**는 자격 증명이 아니라 주소류 설정이므로 플랫폼 Secret의 optional `k3s-mcp-url`·`plugin-marketplace-url` 키로 재정의할 수 있다 — 각각 `K3S_MCP_URL`·`CLAUDE_CODE_PLUGIN_MARKETPLACE_URL`로 **주 컨테이너에만** optional SecretKeyRef 주입되며 credential-proxy와 control plane에는 주입하지 않는다. 키가 없거나 값이 비면 entrypoint가 프로덕션 기본값(`https://homelab-k3s-mcp.llkm.nl/mcp`와 `https://github.com/dlddu/plugin-marketplace.git`)을 그대로 쓰므로 `k8s/` 매니페스트는 한 줄도 달라지지 않는다 — `ca-cert`와 같은 형태의 optional 주소류 키다. 재정의는 **코드 경로를 바꾸지 않는다**: 같은 `claude plugin marketplace add`가 실행되므로 가리키는 원격은 같은 저장소 모양을 서빙해야 `session-platform@dlddu-plugins` 설치가 성립하고, 재정의된 K3s MCP 주소도 같은 `github_app_installation_token` 호출을 받는다. 단명 GitHub token을 싣는 Git `extraheader`는 **재정의된 marketplace URL 자체에 범위가 걸리므로** 주소를 옮기면 범위도 함께 옮겨가고 다른 호스트로 넓어지지 않는다. 공급자 upstream과 달리 이 두 주소에는 **스킴을 강제하지 않는다** — 시작 시 HTTPS로 고정하는 것은 공급자 자격 증명이 흐르는 proxy 경로이고, 이 두 주소를 재정의하는 쪽은 클러스터 내부 원격처럼 신뢰 구간을 가리키는 배치다(kind e2e SUT가 그렇게 쓴다). 평문 원격을 가리키면 단명 GitHub token이 그 구간을 평문으로 지나므로, 재정의의 전송 신뢰는 그 주소를 고른 배치가 진다.
...
- **달성 가치**: V1
- **검증 방법**: Secret의 필수 HTTPS `base-url`·`auth-token`이 sidecar에만 주입되고 주 컨테이너에는 localhost URL·placeholder와 필수 `k3s-mcp-token` SecretKeyRef만 있음을 확인한다. entrypoint가 Claude workload에서만 K3s MCP에 `plugin-marketplace`/`contents:read` 토큰을 요청하고 HTTPS marketplace와 plugin을 설치한 뒤 GitHub token 없이 agent를 exec하며, shell/proxy는 bootstrap을 건너뛰는지 확인한다. optional `k3s-mcp-url`·`plugin-marketplace-url`이 주 컨테이너에만 optional SecretKeyRef로 있고 credential-proxy·control-plane에는 없음을 확인한다. 두 키에 값이 있으면 부트스트랩이 그 주소로 K3s MCP를 호출하고 그 원격에 `marketplace add`를 실행하며 Git `extraheader`의 범위도 그 URL을 따라 이동하는지, 키가 없거나 값이 비면 프로덕션 기본 주소를 쓰는지 확인한다. session managed settings가 해당 plugin만 활성화하는지 확인한다. `K3S_MCP_TOKEN`은 credential-proxy/control-plane에 없고 runner output redaction 대상인지 검증한다. pod가 `data-plane` ServiceAccount와 token을 사용하고 일반 클러스터 리소스에는 `get/list/watch`가 가능하지만 Secret `get/list/watch`는 거부됨을 확인한다. sidecar가 loopback에만 바인딩되고 평문 HTTP·고정 upstream 외 목적지·허용 목록 밖 요청 헤더·CONNECT/Upgrade를 거부하거나 제거하며, 중간 1xx 응답도 자격 증명을 누출하지 않음을 확인한다. optional `ca-cert`가 credential-proxy 컨테이너에만 optional SecretKeyRef로 있고 주 컨테이너에는 없음을 확인한다. 값이 있으면 그 발급자가 서명한 사설 게이트웨이 upstream에 도달하고, 같은 설정에서 공개 발급 upstream의 검증도 계속 성공하며(신뢰 확대가 기존 신뢰를 약화하지 않는다), 키가 없거나 값이 비면 시스템 풀만 쓰고, 인증서가 하나도 없는 비어 있지 않은 번들은 proxy 시작 실패임을 확인한다. 지연 가능한 테스트 upstream이 첫 safe SSE chunk를 flush한 뒤 EOF를 막아도 proxy client가 release 전에 그 chunk를 받고, token을 response read 여러 개에 나누어도 어느 pre-release chunk에도 원문이 없으며 raw upstream 64 MiB SSE 상한을 full-body buffer 없이 지키는지 확인한다. 생성 요청의 자격 증명 필드는 unknown field로 400 거부되고 세션 조회 응답과 read/SSE 출력에 토큰 문자열이 포함되지 않음을 확인한다. runner에서도 token을 여러 stdout/stderr write에 나누어도 중간 SSE event에 원문이 없어야 한다. `platform-default` 세션의 주 컨테이너만 optional `model` SecretKeyRef를 가지며, Secret 값이 비어 있지 않으면 그 값으로 실행하고 키가 없거나 값이 비면 `--model`을 생략하는지 확인한다. 구체 `model`로 생성한 세션은 literal 환경 변수를 사용해 Secret 기본값보다 우선하고, explicit empty/null/비문자열·앞뒤 공백·invalid pattern은 400임을 확인한다.
