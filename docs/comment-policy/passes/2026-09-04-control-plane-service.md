...
## 2026-09-13 증분 재판정 — 게이트 분리 (fail-closed 케이스 doc)

`TestSnapshotIsRefusedForApprovalGatedWithoutAStrategy` 의 doc 을 새 게이트 기준으로 다시 썼다.
**3줄 판정, 제거 0 · 유지 3**(238 → 239).

이 케이스가 서 있는 배포는 이제 「archive gate 를 꺼 둔 배포」가 아니라 **「이 타입 전용 게이트
(`SESSION_APPROVAL_GATED_ARCHIVE_ENABLED`)를 내렸거나 상속원이 off 인 배포」**다. 셋 다 남기는 이유는
하나다 — **이 테스트는 서비스 계층이라 env 를 아예 모른다.** 조립을 직접 주입받으므로 「무엇을 내리면
이 케이스가 되는가」는 본문 어디에서도 읽히지 않고, 그 연결이 끊기면 이 케이스는 「전략을 등록하지
않으면 거부한다」는 동어반복으로 읽힌다.
