...
5. **마운트의 모드는 아카이브의 것이 아니다** — 공유 루트 엔트리를 건너뛰는 `continue` 는 그 이유를
   적지 않으면 「엔트리를 흘린다」로 읽힌다. 그 모드가 실제로 닿을 자리는 버려지는 스테이징
   디렉터리이고, 소유자 쓰기가 빠진 모드면 그 안의 엔트리들이 갇힌다.
