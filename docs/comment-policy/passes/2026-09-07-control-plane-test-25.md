...
`e2e_f4_helper_pod_test.go` 에서 거부 케이스와 함께 **기존 20줄**이 사라졌고, 그 자리에 초안이
19줄을 지었다. **19줄 판정, 제거 14 · 유지 5**(441 → 426).

**이 행에서 처음으로 제거가 신규보다 기존 쪽에서 크게 나왔다.** 사라진 20줄은 이 파일이 「선행이
풀리면 무엇을 되살려라」를 적어 둔 지시문(`restoreTheFreezeBranches`)과 그 근거, 그리고 503 본문을
import 하지 않고 베낀 이유다. 선행이 이 PR 로 풀렸으므로 그 지시문은 **자기 목적을 다했다** — 그
내용의 영구 보관처는 커밋과 이 원장이지 파일이 아니다. 지시문이 의도대로 작동한 사례이므로 적어 둔다:
초안 저자가 그 문장을 읽고 무엇을 지어야 하는지 알았다.

| 위치 | 제거한 것 | 이미 말하는 곳 (복원 경로) |
| --- | --- | --- |
| 파일 헤더 (4줄 문단) | 「동결·복원 갈래는 아직 관측 불가이며 미검증 분기에 등재돼 있다」의 갱신판 | ③④ 경위는 PR 과 커밋, ② 현재 상태는 `doc-tracker.md` 의 AC-F5 항목 |
| `f4ReadSessionMCPURL` doc | 「스펙이 아니라 **실행 중인 컨테이너**에서 읽는 이유」 | ① 자매 파일 `shared_volume_test.go` 의 `TestSharedVolume_BothContainersAreToldWhereItIs` 헤더가 문장 단위로 같은 것을 이미 소유한다 |
| `f4Switch` doc | 「제품 자신의 경로로 복원한다」 | ① 하네스의 `snapshotSession` doc 이 **동결 방향으로** 같은 것을 적는다 |
| 새 테스트 doc (3줄) | 「동결이 쌍을 회수하고 복원이 새 쌍을 세운다」 | ① 테스트 이름과 본문 |
| 회수 단언 인라인 (2→1) | 「API 먼저, 그다음 클러스터의 그라운드-트루스」 | ① 두 단언의 순서가 그대로 보인다 |

**유지 5줄** — 전부 **공허하지 않음의 논거**다. 동결 **전에** `SESSION_MCP_URL` 을 재는 이유(그래야
「다시 배선됐다」가 파드 재생성에 대한 가정이 아니라 측정이 된다)와, 복원된 워크로드가 **회수된 헬퍼의
주소를 그대로 들고 있어도** 위의 모든 단언이 통과한다는 것. 후자가 없으면 마지막 두 비교가 군더더기로
읽혀 다음 판정에서 걷힌다.

**`shared_volume_test.go` 는 이 행에 편입하지 않는다.** 9~13차가 그 파일을 유예한 사유인 「다른
슬라이스의 살아 있는 소유 선언」이 바로 이 슬라이스였고, 이 PR 이 그 선언을 걷는다. 미등재 파일을
판정하는 것은 선언이 사라진 **뒤에** 자매 모델이 여는 패스의 몫이고, 지금 편입하면 남의 슬라이스가
남긴 기존 주석까지 이 행이 판정했다고 주장하게 된다.
