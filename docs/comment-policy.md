...
| 2026-09-04 | `control-plane/internal/adapter/k8s/client_orchestrator.go` · `control-plane/internal/adapter/k8s/orchestrator.go` · `control-plane/internal/adapter/k8s/network_policy.go` · `control-plane/internal/adapter/k8s/orchestrator_test.go` · `control-plane/internal/adapter/k8s/shared_volume.go` | 377 | `5e7b3267006e` | 3차 판정 패스 — **473줄 판정, 제거 153 · 유지 320**. 이후 #66이 더한 14줄을 재판정(제거 6 · 유지 8) — 누적 **487줄 판정, 제거 159 · 유지 328**. *2026-09-07 증분 재판정: AC-F5 공유 볼륨 슬라이스. `client_orchestrator.go`에 더한 **16줄 판정 — 제거 8 · 유지 8** (마운트 자리의 AC 축자와 `applySharedVolumeClaim`/`applySessionNetworkPolicies` doc의 사본을 걷었다). 같은 슬라이스의 신규 `shared_volume.go`를 이 행에 편입해 판정 — 초안의 패키지 doc·상수 주석에서 AC-F5 축자와 이웃 함수 doc의 사본을 걷어 **41줄**로 남겼다(남긴 것: EmptyDir·hostPath를 왜 배제했는가, `StorageClassName` 미설정과 빈 문자열의 차이, 마운트 경로를 최상위에 둔 이유 — 셋 다 레포 어디에도 없다). 328 → 377.* 상세 ↓ |
...
| 2026-09-05 | `control-plane/cmd/control-plane/main.go` | 62 | `ff5f2db629ae` | 6차 판정 패스 — **77줄 판정, 제거 19 · 유지 58**. 4차 패스가 「다음 패스의 1순위」로 지목한 조립 루트. **거짓이 된 주석 2건**(「helper pod's session MCP는 아직 구현되지 않았다」)을 포함한다. *2026-09-07 증분 재판정: AC-F5 공유 볼륨 슬라이스가 더한 **9줄 판정 — 제거 5 · 유지 4**. 걷은 것은 옵션 자리와 config 필드의 「어느 StorageClass가 ReadWriteMany를 내주는지는 클러스터의 성질이다」 — `WithSharedVolume` doc이 정본이다. 남긴 것은 크기 파싱이 **왜 조용한 폴백이 아니라 기동 실패인가**로, 바로 아래 `envDuration`/`envBool`이 반대로 동작해 대비가 코드에 안 보인다. 58 → 62.* 상세 ↓ |
