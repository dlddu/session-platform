...
- **다른 여정의 대상**은 그 여정의 페이지가 이미 있으면 `../JRN-y/#STP-x` 로 링크한다.
  아직 없으면 `data-branch-pending` 을 달아 **그 사실을 명시**한다 — 이때 누르면 페이지 안에서
  `[data-state-note]` 하나가 보이며 "이 갈래는 아직 페이지가 없다"를 알린다.
  대상 페이지가 생기는 순간 `data-branch-pending` 은 하네스에서 **실패**가 된다(링크로 승급해야 한다).
